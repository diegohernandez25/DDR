...
energy_worst_link_load = w(w_index)./(sum(sum(C))/2);
removed_links = [];
...
while( worst_link_load <= w(w_index))
    ...
    for index = 1:numLinks %percorro todos os links disponiveis na rede
        ...
        %FOR PART 2!
        energy_calculated_worst_link_load = calculated_worst_link_load./(sum(sum(C))/2);
        if(energy_calculated_worst_link_load < best_energy_calculated_worst_link_load && calculated_worst_link_load < w(w_index))
           ...
           %store removed link that gets best worst link load.
           linkIndex = index;
        end
    end
    ...
    energy_worst_link_load = best_energy_calculated_worst_link_load;
...
fprintf("final energy worst link load: %d\n",energy_worst_link_load);
